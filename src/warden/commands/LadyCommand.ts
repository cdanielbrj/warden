...
export function createLadyCommand(
  client: Client,
  registry: LadyRegistryService,
): WardenCommand {
  ...
  return {
    data: new SlashCommandBuilder()
      .setName("lady")
      .setDescription("Show one managed Lady")
      .addStringOption((option) =>
        option
          .setName("id")
          .setDescription("Lady ID")
          .setRequired(true)
    ...
    async autocomplete(interaction) {
      ...
      const choices = registry
        .list()
        .filter((lady) => lady.id.includes(query))
        .slice(0, 25)
        .map((lady) => ({ name: lady.id, value: lady.id }));
    ...
    async execute(interaction) {
      ...
      const overview = await overviews.getOverview(
        interaction.options.getString("id", true),
        { includePlayers: true },
      );

      if (!overview) {
        await interaction.editReply("Lady is not registered.");
        return;
      }

      await completeCommandEmbeds(
        interaction,
        visibility,
        await embeds.create([overview]),
      );
    },
  };
}
