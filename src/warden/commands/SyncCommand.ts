...
export function createSyncCommand(registry: LadyRegistryService): WardenCommand {
  ...
  return {
    data: new SlashCommandBuilder()
      .setName("sync")
      .setDescription("Synchronize the registered Ladies")
    ...
    async execute(interaction) {
      ...
      const results = await overviews.getOverviews({ includePlayers: false });
      const online = results.filter(
        (result) => result.status?.gameStatus === "online",
      ).length;

      await completeCommand(
        interaction,
        visibility,
        `Synchronized ${online}/${results.length} registered Ladies.`,
      );
    },
  };
}
